...
<div class="card">
    ...
    <table class="table table-hover align-middle shadow-lg rounded"
        ...
        <thead style="background: #2C3E50; color: white;">
            <tr>
                <th class="text-center px-4 py-3" style="border-top-left-radius: 12px;">{{ __('productType.ID') }}</th>
                <th class="text-start px-4 py-3">{{ __('productType.Name') }}</th>
                <th class="text-start px-4 py-3">{{ __('productType.Category_Name') }}</th>
                <th class="text-center px-4 py-3">{{ __('common.Created_At') }}</th>
                <th class="text-center px-4 py-3" style="border-top-right-radius: 12px;">{{ __('productType.Actions') }}</th>
            </tr>
        </thead>
        ...
        <tbody>
            ...
            <tr class="transition"
                ...
                <td class="text-start fw-semibold px-4 py-3 ">{{ $productType->category ? $productType->category->name : 'N/A' }}</td>
    ...
    <div class="d-flex justify-content-center align-items-center flex-wrap" style="gap: 6px;">
        ...
        <button class="btn btn-sm px-3 py-1 shadow-sm rounded-pill btn-delete"
            data-url="{{ route('productTypes.destroy', $productType) }}"
