...
@section('title', __('brand.Update'))
@section('content-header', __('brand.Update'))
            ...
            {{-- <div class="form-group">
                ...
                <select name="product_type_id" id="product_type_id" class="form-control @error('product_type_id') is-invalid @enderror">
                    ...
                    @foreach($productTypes as $productType)
                        <option value="{{ $productType->id }}" {{ old('product_type_id', $brand->product_type_id) == $productType->id ? 'selected' : '' }}>
                            {{ $productType->name }}
                        </option>
                    @endforeach
                </select>
